...
class LTXPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def tensor_to_vae_latent(self, video_tensor):
        ...
        # print("video_tensor", video_tensor.shape)
        # print("video_tensor.shape", video_tensor[0].shape)
        # import pdb; pdb.set_trace()
        # batch_size, num_channels, num_frames, height, width = x.shape
        # import pdb; pdb.set_trace()
        init_latents = [
            retrieve_latents(self.vae.encode(img.unsqueeze(0)), None, sample_mode="argmax") for img in video_tensor
        ]

        init_latents = torch.cat(init_latents, dim=0).to(self.transformer.dtype)
        init_latents = self._normalize_latents(init_latents, self.vae.latents_mean, self.vae.latents_std)
