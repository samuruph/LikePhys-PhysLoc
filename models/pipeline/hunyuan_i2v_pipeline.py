...
class HunyuanVideoImageToVideoPipeline(DiffusionPipeline, HunyuanVideoLoraLoaderMixin):
    ...
    # Encode Prompt for T2V
    def _get_llama_prompt_embeds_t2v(
        self,
        prompt: Union[str, List[str]],
        prompt_template: Dict[str, Any],
        num_videos_per_prompt: int = 1,
    ...
    def _get_clip_prompt_embeds_t2v(
        self,
        prompt: Union[str, List[str]],
        num_videos_per_prompt: int = 1,
        device: Optional[torch.device] = None,
    ...
    def encode_prompt_t2v(
        ...
        if prompt_embeds is None:
            prompt_embeds, prompt_attention_mask = self._get_llama_prompt_embeds_t2v(
                prompt,
                prompt_template,
                num_videos_per_prompt,
                device=device,
        ...
        if pooled_prompt_embeds is None:
            if prompt_2 is None:
                prompt_2 = prompt
            pooled_prompt_embeds = self._get_clip_prompt_embeds_t2v(
                prompt,
                num_videos_per_prompt,
                device=device,
                dtype=dtype,
    ...
    def tensor_to_vae_latent(self, video_tensor):
        # image = image.unsqueeze(2)  # [B, C, 1, H, W]
        video_tensor = video_tensor.permute(0, 2, 1, 3, 4)  # [B, C, 1, H, W]
        # print("video_tensor", video_tensor.shape)
        image_latents = [retrieve_latents(self.vae.encode(img.unsqueeze(0)), None, "argmax") for img in video_tensor]
        
        image_latents = torch.cat(image_latents, dim=0).to(self.transformer.dtype) * self.vae_scaling_factor
