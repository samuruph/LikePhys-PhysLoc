...
class MochiPipeline(DiffusionPipeline, Mochi1LoraLoaderMixin):
    ...
    def tensor_to_vae_latent(self, video_tensor):
        ...
        init_latents = [retrieve_latents(self.vae.encode(img.unsqueeze(0)), None, "argmax") for img in video_tensor]
        ...
        if has_latents_mean and has_latents_std:
            latents_mean = (
                torch.tensor(self.vae.config.latents_mean).view(1, 12, 1, 1, 1).to(init_latents.device, init_latents.dtype)
            )
            latents_std = (
                torch.tensor(self.vae.config.latents_std).view(1, 12, 1, 1, 1).to(init_latents.device, init_latents.dtype)
            )
